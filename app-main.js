...
const configuredOdysseyKeyProxyUrl = String(
    window.WANDER_ODYSSEY_KEY_PROXY_URL ||
    (wanderApiBase ? `${wanderApiBase}/odyssey-demo-key` : '')
).trim();
const odysseyKeyProxyUrl = isLocalRuntimeHost && window.ODYSSEY_API_KEY ? '' : configuredOdysseyKeyProxyUrl;
const configuredOdysseyApiKey = window.ODYSSEY_API_KEY || '';
const configuredOdysseyModel = String(window.WANDER_ODYSSEY_MODEL || '').trim();
const allowOdysseyKeyPrompt = window.WANDER_ALLOW_ODYSSEY_KEY_PROMPT !== false;
const DEFAULT_PROJECT_TITLE = 'untitled project';
const ODYSSEY_MODEL_STORAGE_KEY = 'wander_odyssey_model';
...
function getOdysseyStreamModel() {
    return String(localStorage.getItem(ODYSSEY_MODEL_STORAGE_KEY) || configuredOdysseyModel || '').trim();
}

function persistOdysseyModel(model) {
    const normalizedModel = String(model || '').trim();
    if (normalizedModel) {
        localStorage.setItem(ODYSSEY_MODEL_STORAGE_KEY, normalizedModel);
    } else {
        localStorage.removeItem(ODYSSEY_MODEL_STORAGE_KEY);
    }
...
function buildOdysseyStartOptions(prompt, options = {}) {
    ...
    const model = getOdysseyStreamModel();
    if (model) {
        payload.model = model;
    }
    return payload;
...
function requestOdysseyApiKeyFromUser() {
    showOdysseyKeyGate(odysseyKeyProxyUrl ? 'Enter the demo password to start streaming.' : 'Enter an Odyssey API key to start streaming.');
    return '';
}
...
async function fetchOdysseyDemoCredentials(password) {
    if (!odysseyKeyProxyUrl) {
        return {
            apiKey: password,
            model: configuredOdysseyModel
        };
    }

    const response = await fetch(odysseyKeyProxyUrl, {
        method: 'POST',
        headers: {
    ...
    return {
        apiKey: String(data?.odysseyApiKey || data?.apiKey || '').trim(),
...
function setupOdysseyKeyGate() {
    ...
    const startWithEnteredKey = async () => {
        ...
        if (!enteredValue) {
            setOdysseyKeyGateError(odysseyKeyProxyUrl ? 'Enter the demo password first.' : 'Paste a valid Odyssey API key first.');
            odysseyKeyInputEl.focus();
            return;
        }

        const credentials = await fetchOdysseyDemoCredentials(enteredValue);
        const apiKey = persistOdysseyApiKey(credentials.apiKey);
        if (!apiKey) {
            setOdysseyKeyGateError('Demo access did not return an Odyssey key.');
            odysseyKeyInputEl.focus();
            return;
        }
        persistOdysseyModel(credentials.model || configuredOdysseyModel);
...
async function connectCardClientWithRetry(cardState, apiKey, connectOptions) {
    ...
    for (let attempt = 1; attempt <= ODYSSEY_CONNECT_MAX_ATTEMPTS; attempt += 1) {
        const client = new window.Odyssey({ apiKey });
        cardState.client = client;
        try {
            const mediaStream = await client.connect(connectOptions);
    ...
    if (isOdysseyConcurrentSessionError(lastError)) {
        ...
        await forceCloseAllOdysseySessions({ excludeCardId: cardState.id, trackProjectChange: false });
        const emergencyClient = new window.Odyssey({ apiKey });
        cardState.client = emergencyClient;
        try {
            const mediaStream = await emergencyClient.connect(connectOptions);
...
async function restartStreamCard(cardState) {
    ...
    const apiKey = getOrRequestOdysseyApiKey();
    if (!apiKey) {
        throw new Error('Enter an Odyssey API key to resume this stream.');
    }
...
async function enterAppFromLanding() {
    ...
    if (!apiKey) {
        showOdysseyKeyGate('Enter an Odyssey API key to start.');
        return;
    }
